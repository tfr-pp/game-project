...
{
	class LevelSelectMenu(SpriteFont font, IList<string> levels, Action<string> onSelect, Action onBack = null) : IMenu
	{
		private readonly SpriteFont font = font;
		private readonly IList<string> levels = levels ?? [];
		...
		public void Draw(SpriteBatch sb)
		{
			...
			sb.DrawString(font, "Sélectionner un niveau", new Vector2(centerX - font.MeasureString("Sélectionner un niveau").X / 2, startY - 40), Color.CornflowerBlue);
			for (int i = 0; i < levels.Count; i++)
			{
				var text = levels[i];
				var colour = i == selected ? Color.Yellow : Color.White;
				var size = font.MeasureString(text);
				sb.DrawString(font, text, new Vector2(centerX - size.X / 2, startY + i * (size.Y + 8)), colour);
			}
		}
